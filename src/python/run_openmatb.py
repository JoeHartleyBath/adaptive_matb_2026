...
def main() -> int:
    ...
    scenario_filename = "pilot_dry_run_v0.txt" if args.dry_run else scenario_by_seq_id[seq_id]
    scenario_target_path = openmatb_dir / "includes" / "scenarios" / scenario_filename
    if not scenario_target_path.exists():
        print(
            f"Scenario file for {seq_id} not found: {scenario_target_path}. "
            "Author the scenario file before running.",
            file=sys.stderr,
        )
        return 2
    ...
    # Preserve for downstream tooling and manifest post-processing.
    os.environ["OPENMATB_SEQ_ID"] = seq_id

    os.environ["OPENMATB_UNATTENDED"] = "1" if args.unattended else "0"
    if args.dry_run_timeout_seconds is not None:
        if args.dry_run_timeout_seconds <= 0:
            print("--dry-run-timeout-seconds must be a positive integer", file=sys.stderr)
            return 2
        if args.unattended:
            os.environ["OPENMATB_DRY_RUN_TIMEOUT_SECONDS"] = str(args.dry_run_timeout_seconds)
    if args.unattended:
        print("WARNING: Unattended mode is ON. Inputs, instructions, and TLX will be auto-skipped.")

    sessions_dir = output_root_path / Path(os.environ["OPENMATB_OUTPUT_SUBDIR"]) / "sessions"
    ...
    proc = subprocess.Popen([sys.executable, "-c", bootstrap], cwd=str(openmatb_dir))
    if timeout_seconds is None:
        ...
        while True:
            ...
            if elapsed > timeout_seconds and not timeout_triggered:
                ...
                print(f"DRY RUN TIMEOUT: aborting after {timeout_seconds}s")
    ...
    if len(new_manifests) != 1:
        print(
            "Unable to unambiguously locate the new run manifest to record seq_id. "
            f"Expected 1 new manifest in {sessions_dir}, found {len(new_manifests)}.",
            file=sys.stderr,
        )
        return 2
