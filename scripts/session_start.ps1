...
while ($true) {
    if ($Pilot) {
        $Input = Read-Host "  Enter PILOT number (1-10)"
    } else {
        $Input = Read-Host "  Enter PARTICIPANT number (1-30)"
    }

    $Input = $Input.Trim()

    if (-not ($Input -match '^\d+$')) {
        Write-Host "  Please enter a number only (e.g. 12)." -ForegroundColor Yellow
        continue
    }
